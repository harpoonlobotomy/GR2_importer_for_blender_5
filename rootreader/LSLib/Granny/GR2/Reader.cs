...
public class GR2Reader(Stream stream) : IDisposable
{
    ...
    private Header ReadHeader()
    {
        var header = new Header
        {
            ...
            rootNode = ReadSectionReferenceUnchecked(), // last build removed this line. Have temporarily put it back.
            ...
            foreach (var member in definition.Members)
            {
                ...
                else if (member.TotalMarshallingSize() > 1)
                {
                    ...
                    for (var j = 0; j < size / marshalSize; j++)
                    {
                        ...
                        {
                            (data[j * marshalSize + marshalSize - 1 - off], data[j * marshalSize + off]) = (data[j * marshalSize + off], data[j * marshalSize + marshalSize - 1 - off]);
                        }
                    }
    ...
    internal object ReadStruct(StructDefinition definition, MemberType memberType, object node, object parent)
    {
        ...
        if (memberType != MemberType.Inline && CachedStructs.TryGetValue(offset, out object cachedNode))
        {
#if DEBUG_GR2_SERIALIZATION
    ...
    internal object ReadInstance(MemberDefinition definition, object node, Type propertyType, object parent)
    {
        ...
        if (propertyType != null)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < definition.ArraySize; i++)
                {
                    ...
                    //Console.WriteLine("items.Add(ReadElement("); // UNUSED?
        ...
        else
        {
            ...
            //Console.WriteLine("else readElement");
            //Console.Write(definition); // rootreader.LSLib.Granny.GR2.MemberDefinitionS x4
            return null;
        }
    }
...
#if DEBUG_GR2_SERIALIZATION
        if (node != null)
            Debug.WriteLine(String.Format("    [{0:X8}] {1}: {2}", definition.Name, node.ToString()));
        else
            Debug.WriteLine(String.Format("    [{0:X8}] {1}: <null>", definition.Name));
#endif
